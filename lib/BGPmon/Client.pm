...
# will save memory.
our %EXPORT_TAGS = ( 'all' => [ qw(new connect address port xml_handler 
  xml_handler_params run clear_errors) ] );
...
sub run{
  ...
  if(!defined($self->{sel})){
    $self->add_error("Select is undef in running --> was connect called ".
      "successfully?\n");
    return 0;
  }
  my $start_time = time;
  ...
  {
    #-----------------------------
    # Read from each ready socket: this is in a loop in case we ever expand 
    # connect to be used with more than one socket. as of now there will be 0 
    # or one of these only. If multiple sockets are added we will need to add 
    # multiple xml_handlers, one corresponding to each handler.
    #-----------------------------
    foreach my $fh (@ready)
    {
        my $text      = "";
...
This code is meant to help write BGPmon clients. It is the listening code for 
a single socket connection with a single filehandle. 
It is meant to be accompanied by one of the parsing helper classes also defined
in this package. The parsing helper class should define the function pointer 
sent into the xml_handler.
